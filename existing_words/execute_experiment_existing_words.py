...
def read_list_of_batches(run_prefix: str) -> list:
    file_names = [
        file
        for file in os.listdir(f"./batches")
...
def execute_tasks_openai(list_of_batch_names: list) -> list:
    list_of_job_ids = []
    for file_name in list_of_batch_names:
        batch_file = client.files.create(
            file=open(f"batches/{file_name}", "rb"), purpose="batch"
        )
...
def save_results_openai(list_of_job_ids: list, run_prefix: str):
    ...
    for index, batch_job_id in enumerate(list_of_job_ids):
        ...
        with open(
            f"results/{run_prefix}_results_{index}_{date_string}.jsonl",
            "wb",
        ) as file:
            file.write(result)
...
def execute_tasks_google(list_of_batch_names: list) -> list:
    if config_args.get("model_name").startswith('projects'):
        ...
        for file_name in list_of_batch_names:
            ...
            LOCAL_FILE_original = f"batches/{file_name}"
            LOCAL_FILE = f"batches/Add_role_{file_name}"
        ...
        for file_name in list_of_batch_names:
            uploaded_file = client.files.upload(
                file=open(f"batches/{file_name}", "rb"),
...
def format_results_huggingface(output, tokenizer, counter, logprobs=5):
...
def execute_tasks_save_model_hf_or_local(list_of_batch_names: list, run_prefix: str, batch_size=5) -> list:
    list_of_job_names = []
    ...
    for index, file_name in enumerate(list_of_batch_names):
        ...
        date_string = datetime.now().strftime("%Y-%m-%d-%H-%M")
        os.makedirs(f"results", exist_ok=True)
        output_file = f"results/{run_prefix}_results_{index+1}_{date_string}.jsonl"
                ...
                for obj in tqdm(reader, desc=f"Processing {jsonl_file_path}"):
                    ...
                    if len(batch_messages) == batch_size:
                        ...
                        for output in outputs:
                            counter += 1
                            json_line = format_results_huggingface(output, tokenizer, counter, logprobs)
...
def save_results_google(list_of_job_names: list, run_prefix: str):
    os.makedirs(f"results", exist_ok=True)
    date_string = datetime.now().strftime("%Y-%m-%d-%H-%M")
    ...
    for index, batch_job_name in enumerate(list_of_job_names):
        ...
        if batch_job.state.name == 'JOB_STATE_SUCCEEDED':
            if batch_job.dest and batch_job.dest.file_name:
                ...
                save_path = f"results/{run_prefix}_results_{index}_{date_string}.jsonl"
...
def save_results_google_finetuned(jobs: list, run_prefix: str):
    os.makedirs(f"results", exist_ok=True)
    date_string = datetime.now().strftime("%Y-%m-%d-%H-%M")
...
if __name__ == "__main__":
    ...
    if company == "OpenAI":
        ...
        list_of_job_ids = execute_tasks_openai(
            list_of_batch_names=list_of_batch_names
        )
        ...
        execute_tasks_save_model_hf_or_local(
            list_of_batch_names=list_of_batch_names, run_prefix=EXPERIMENT_NAME
        )
    elif company == "Local":
        execute_tasks_save_model_hf_or_local(
            list_of_batch_names=list_of_batch_names, run_prefix=EXPERIMENT_NAME
